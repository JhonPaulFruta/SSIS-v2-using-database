...
class Home(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="MSU-IIT STUDENT INFORMATION SYSTEM", font=("Monaco", 30, 'bold'),bd=0, width = 60, bg="goldenrod", fg="maroon",)
        label.place(x=0,y=20)

        home = tk.Button(self, text="HOME",font=("Monaco",16,"bold"),bd=0, width = 13, bg="goldenrod", fg="maroon", command=lambda: controller.show(Home))
        home.place(x=380,y=200)
        home.config(cursor= "hand2")
        
        course = tk.Button(self, text="COURSES",font=("Monaco",16,"bold"),bd=0, width = 10, bg="goldenrod", fg="maroon", command=lambda: controller.show(Courses))
        course.place(x=700,y=200)
        course.config(cursor= "hand2")
        
        students = tk.Button(self, text="REGISTRATION",font=("Monaco",16,"bold"),bd=0, width = 13, bg="goldenrod", fg="maroon", command=lambda: controller.show(Students))
        students.place(x=380,y=300)
        students.config(cursor= "hand2")

        students = tk.Button(self, text="EXIT",font=("Monaco",15,"bold"),bd=0, width = 11, bg="goldenrod", fg="maroon", command=iExit)
...
class Courses(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="COURSES ", font=("Monaco", 40, 'bold'),bd=0, width = 10, bg="goldenrod", fg="maroon",)
        ...
        home = tk.Button(self, text="HOME",font=("Monaco",13,"bold"),bd=0, width = 10, bg="goldenrod", fg="maroon", command=lambda: controller.show(Home))
        home.place(x=1,y=35)
        home.config(cursor= "hand2")
        ...
        self.lblccode = Label(self, font=("Monaco", 12, "bold"), text="Course Code:", padx=5, pady=5)
        self.lblccode.place(x=125,y=144)
        self.txtccode = Entry(self, font=("Monaco", 13), textvariable=Course_Code, width=31)
        self.txtccode.place(x=270,y=150)

        self.lblcname = Label(self, font=("Monaco", 12,"bold"), text="Course Name:", padx=5, pady=5)
        self.lblcname.place(x=125,y=205)
        self.txtcname = Entry(self, font=("Monaco", 13), textvariable=Course_Name, width=31)
        self.txtcname.place(x=270,y=210)
        
        self.SearchBar = Entry(self, font=("Monaco", 11), textvariable=SearchBar_Var, bd=0,bg="Gray",width=31)
        ...
        self.adds = Button(self, text="ADD", font=('Monaco', 11, ), height=2, width=10, bd=1, bg="goldenrod", fg="maroon",command=add_course)
        self.adds.place(x=150,y=300)
        self.adds.config(cursor= "hand2")

        self.update = Button(self, text="UPDATE", font=('Monaco', 11), height=2, width=10, bd=1, bg="goldenrod", fg="maroon", command=update_course) 
        self.update.place(x=150,y=360)
        self.update.config(cursor= "hand2")

        self.clear = Button(self, text="CLEAR", font=('Monaco', 11), height=2, width=10, bd=1, bg="goldenrod", fg="maroon", command=clear)
        self.clear.place(x=350,y=300)
        self.clear.config(cursor= "hand2")

        self.delete = Button(self, text="DELETE", font=('Monaco', 11), height=2, width=10, bd=1, bg="goldenrod", fg="maroon", command=delete_course)
        self.delete.place(x=350,y=358)
        self.delete.config(cursor= "hand2")

        self.search = Button(self, text="SEARCH", font=('Monaco', 11, 'bold'),bd=0, bg= "goldenrod", fg="maroon", command=search_course)
        self.search.place(x=1160,y=100)
        self.search.config(cursor= "hand2")

        self.display = Button(self, text="DISPLAY", font=('Monaco', 11, 'bold'), height=1, width=11, bg="goldenrod", fg="maroon", command=display_course)
...
class Students(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="REGISTRATION FORM ", font=("Monaco", 40, 'bold'),bd=0, width = 20, bg="goldenrod", fg="maroon",)
        label.place(x=130,y=20)
        
        home = tk.Button(self, text="HOME",font=("Monaco",12,"bold"),bd=0, width = 7, bg="goldenrod", fg="maroon",  command=lambda: controller.show(Home))
        home.place(x=1,y=35)
        home.config(cursor= "hand2")
        ...
        self.lblid = Label(self, font=("Monaco", 12,), text="ID Number:", padx=5, pady=5)
        self.lblid.place(x=125,y=144)
        self.txtid = Entry(self, font=("Monaco", 13), textvariable=Student_ID, width=33)
        self.txtid.place(x=255,y=150)

        self.lblname = Label(self, font=("Monaco", 12,), text="Name:", padx=5, pady=5)
        self.lblname.place(x=125,y=205)
        self.txtname = Entry(self, font=("Monaco", 13), textvariable=Student_Name, width=33)
        self.txtname.place(x=255,y=210)
        
        self.lblc = Label(self, font=("Monaco", 12,), text="Course Code:", padx=5, pady=5)
        self.lblc.place(x=125,y=269)
        self.txtyear = ttk.Combobox(self, value=["BS STAT", "BS MATH", "BS IT", "BS CS","BSA","BS Bio","BS Chem","BSCE","BSHM","BS Micr","BA Hist","BAPhil","BA PoSc", "B.S.B.A.", "B.S.Ed.", "B.S.N.", "B.F.A", "BA Hist", "BA PolSci", "BA Soc", "BA Fil", "BA Eng", "BS CE", "BS Psych"], state="readonly", font=("Monaco", 12), textvariable=Course_Code, width=28)
        self.txtyear.place(x=255,y=274)
        

        self.lblyear = Label(self, font=("Monaco", 12,), text="Year Level:", padx=5, pady=5)
        self.lblyear.place(x=125,y=315)
        self.txtyear = ttk.Combobox(self, value=["1st Year", "2nd Year", "3rd Year", "4th Year"], state="readonly", font=("Monaco", 13), textvariable=Student_YearLevel, width=31)
        self.txtyear.place(x=255,y=320)
        
        self.lblgender = Label(self, font=("Monaco", 13,), text="Gender:", padx=5, pady=5)
        self.lblgender.place(x=125,y=361)
        self.txtgender = ttk.Combobox(self, value=["Male", "Female"], font=("Monaco", 13), state="readonly", textvariable=Student_Gender, width=28)
        ...
        self.add = Button(self, text="REGISTER", font=('Monaco', 11), height=1, width=10, bd=1,  bg="goldenrod", fg="maroon", command=add_stud)
        self.add.place(x=150,y=420)
        self.add.config(cursor= "hand2")

        self.update = Button(self, text="UPDATE", font=('Monaco', 11), height=1, width=10, bd=1, bg="goldenrod", fg="maroon", command=update_stud)
        self.update.place(x=250,y=420)
        self.update.config(cursor= "hand2")

        self.clear = Button(self, text="CLEAR", font=('Monaco', 11), height=1, width=10, bd=1, bg="goldenrod", fg="maroon", command=clear)
        self.clear.place(x=350,y=420)
        self.clear.config(cursor= "hand2")

        self.delete = Button(self, text="DELETE", font=('Monaco', 11), height=1, width=10, bd=1, bg="goldenrod", fg="maroon", command=delete_stud)
        self.delete.place(x=450,y=420)
        self.delete.config(cursor= "hand2")

        self.search = Button(self, text="SEARCH", font=('Monaco', 11, 'bold'),bd=0, bg= 'goldenrod', fg="maroon", command=search_stud)
        self.search.place(x=1160,y=100)
        self.search.config(cursor= "hand2")

        self.display = Button(self, text="DISPLAY", font=('Monaco', 11, 'bold'), height=1, width=11,  bg="goldenrod", fg="maroon",command = display_stud)
        self.display.place(x=575,y=105)
        self.display.config(cursor= "hand2")
